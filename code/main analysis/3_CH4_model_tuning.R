...
#trying now with gradient boosting
xgboost_model <- 
  parsnip::boost_tree(
    mode = "regression",
    trees = tune(),
...
xgboost_tuned %>% 
  collect_metrics() %>%
  filter(.metric == "rsq", loss_reduction< .001) %>%
  ggplot(aes(loss_reduction, trees, color = mean)) +
  geom_point()+
...
#xheck perfomance with teaining data
...
ggplot(preds_comp, aes(prediction, observed- prediction))+
  geom_point()+
  facet_wrap(~model)+
